...
@Component({
  ...
  templateUrl: './upload-material-by-student.page.html',
  styleUrl: './upload-material-by-student.page.scss'
})
export class UploadMaterialByStudentPage implements OnInit, OnDestroy {
  ...
  public loggedInStudentRole: string = null;
  ...
  private studentDetailsSubscription(): void {
    this.subscription$.add(
      this.studentDetailsSelected$.subscribe(state => {
        const loggedInStudent = state?.[LOGGED_IN_STUDENT_KEY];

        this.loggedInStudentRole = loggedInStudent?.role ?? null;
      })
    );
  }
    ...
    this.subscription$.add(
      this.materialDetailsState$.subscribe(state => {
        ...
        if (state?.status === MaterialStatusEnum.uploadSuccess) {
          ...
          const successMessage = this.loggedInStudentRole === 'STUDENT'
            ? 'Your upload request was sent successfully. The material will appear after it is reviewed and approved.'
